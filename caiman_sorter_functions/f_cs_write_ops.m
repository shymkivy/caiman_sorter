function f_cs_write_ops(app, ops)
    ...
    if isfield(ops, 'deconv')
        ...
        if isfield(ops.deconv, 'smooth_dfdt')
            try
                ...
                app.GaussKernelSizeFramesSmoothdfdt.Value =                 deconv.smooth_dfdt.params.gauss_kernel_size;
        ...
        if isobject(app.ARmodelSwitchCfoopsi)
            if isfield(ops.deconv, 'c_foopsi')
                try
                    ...
                    app.GaussKernelSizeFramesCfoopsi.Value =                deconv.c_foopsi.params.gauss_kernel_size;
        ...
        if isobject(app.PlotMCMCSwitch)
            if isfield(ops.deconv, 'MCMC')
                try
                    ...
                    app.GaussKernelSizeFramesMCMC.Value =                   deconv.MCMC.params.gauss_kernel_size;
