function proc = f_cs_initialize_new_proc(app)

%% 
proc.idx_components = app.est.idx_components;
proc.idx_components_bad = app.est.idx_components_bad;
proc.dims = app.est.dims;

[proc.num_cells, proc.num_frames] = size(app.est.C);
...
bin_size = floor((proc.peak_bin_size * double(app.ops.init_params_caiman.data.fr))/2);

proc.peaks_ave = zeros(proc.num_cells,1);
temp_peaks = zeros(proc.peaks_to_ave,1);
for n_cell = 1:proc.num_cells
    temp_C = app.est.C(n_cell,:);
...
for n_cell = 1:proc.num_cells
    temp_cell = app.est.YrA(n_cell,:)+app.est.C(n_cell,:);
...
%%
f_cs_update_log(app, 'Computing firing stability...');
proc.firing_stab_vals = f_cs_compute_firing_stability(app.est, proc, app.ops);
