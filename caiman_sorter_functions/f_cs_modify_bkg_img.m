function f_cs_modify_bkg_img(app, add_accept_comp, current_cell_num)
    ...
    if strcmp(temp_text, 'Weighted comp')
        % Use the parameter, not app.current_cell_num — they happen to
        % coincide today but the function signature is a lie otherwise.
        temp_comp = temp_comp * app.bkg_comp_weights(current_cell_num);
    end
